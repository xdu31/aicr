...
if [[ "${SKIP_PREFLIGHT}" == "true" ]]; then
  ...
  if skip_preflight_for_release "foo"; then
    echo "  Skipping foo (privileged-foo): bundle deletes dependent manifests before controller uninstall."
  else
    echo "  Checking foo (privileged-foo)..."
    check_release_for_stuck_crds "foo" "privileged-foo"
  fi
...
for dir in $(ls -1d "${SCRIPT_DIR}"/[0-9][0-9][0-9]-*/ 2>/dev/null | sort -r); do
  ...
  ns=""
  if [[ "${name}" == "foo" ]]; then ns="privileged-foo"; fi
  # Injected mixed-component "-post" folders share their parent's namespace.
  if [[ "${name}" == "foo-post" ]]; then ns="privileged-foo"; fi
  ...
  if [[ "${name}" == "foo-pre" ]]; then ns="privileged-foo"; fi
  if [[ -z "${ns}" ]]; then
...
kubectl get crd -o json 2>/dev/null \
  | jq -r --arg rel "foo" --arg ns "privileged-foo" \
    '.items[] | select(.metadata.annotations["meta.helm.sh/release-name"]==$rel and .metadata.annotations["meta.helm.sh/release-namespace"]==$ns) | .metadata.name' 2>/dev/null \
  | while read -r name; do
      echo "Deleting CRD ${name} (owned by foo/privileged-foo)..."
      kubectl delete crd "${name}" --ignore-not-found --wait=false \
        || echo "Warning: failed to delete CRD ${name} (owned by foo/privileged-foo); leftovers will surface in post-flight" >&2
    done || echo "Warning: orphan-CRD cleanup for foo/privileged-foo failed (kubectl get / jq error); leftovers will surface in post-flight" >&2
...
if [[ "${DELETE_PVCS}" == "true" ]] && ! echo " ${PROTECTED_NS} " | grep -q " privileged-foo "; then
  echo "Deleting PVCs in privileged-foo..."
  kubectl delete pvc --all -n privileged-foo --ignore-not-found || true
fi
delete_orphaned_webhooks_for_ns "privileged-foo"
delete_namespace "privileged-foo"
...
if capture_kubectl_json postflight_all_crds_json get crd -o json; then
  ...
  done < <(extra_crds_for_release "foo")
else
