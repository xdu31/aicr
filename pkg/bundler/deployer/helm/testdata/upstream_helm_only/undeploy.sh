...
if [[ "${SKIP_PREFLIGHT}" == "true" ]]; then
  ...
  if skip_preflight_for_release "cert-manager"; then
    echo "  Skipping cert-manager (cert-manager): bundle deletes dependent manifests before controller uninstall."
  else
    echo "  Checking cert-manager (cert-manager)..."
...
for dir in $(ls -1d "${SCRIPT_DIR}"/[0-9][0-9][0-9]-*/ 2>/dev/null | sort -r); do
  ...
  ns=""
  if [[ "${name}" == "cert-manager" ]]; then ns="cert-manager"; fi
  # Injected mixed-component "-post" folders share their parent's namespace.
  if [[ "${name}" == "cert-manager-post" ]]; then ns="cert-manager"; fi
  ...
  if [[ "${name}" == "cert-manager-pre" ]]; then ns="cert-manager"; fi
  if [[ -z "${ns}" ]]; then
...
kubectl get crd -o json 2>/dev/null \
  | jq -r --arg rel "cert-manager" --arg ns "cert-manager" \
    '.items[] | select(.metadata.annotations["meta.helm.sh/release-name"]==$rel and .metadata.annotations["meta.helm.sh/release-namespace"]==$ns) | .metadata.name' 2>/dev/null \
  | while read -r name; do
      echo "Deleting CRD ${name} (owned by cert-manager/cert-manager)..."
      kubectl delete crd "${name}" --ignore-not-found --wait=false \
        || echo "Warning: failed to delete CRD ${name} (owned by cert-manager/cert-manager); leftovers will surface in post-flight" >&2
    done || echo "Warning: orphan-CRD cleanup for cert-manager/cert-manager failed (kubectl get / jq error); leftovers will surface in post-flight" >&2
...
if [[ "${DELETE_PVCS}" == "true" ]] && ! echo " ${PROTECTED_NS} " | grep -q " cert-manager "; then
  echo "Deleting PVCs in cert-manager..."
  kubectl delete pvc --all -n cert-manager --ignore-not-found || true
fi
delete_orphaned_webhooks_for_ns "cert-manager"
...
if capture_kubectl_json postflight_all_crds_json get crd -o json; then
  ...
  done < <(extra_crds_for_release "cert-manager")
else
