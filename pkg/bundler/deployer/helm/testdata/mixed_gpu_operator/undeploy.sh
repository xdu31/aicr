...
echo "Undeploying AICR components (timeout: ${HELM_TIMEOUT}s)..."
echo ""
...
if [[ "${SKIP_PREFLIGHT}" == "true" ]]; then
  ...
  if skip_preflight_for_release "gpu-operator"; then
    echo "  Skipping gpu-operator (privileged-gpu-operator): bundle deletes dependent manifests before controller uninstall."
  else
    echo "  Checking gpu-operator (privileged-gpu-operator)..."
    check_release_for_stuck_crds "gpu-operator" "privileged-gpu-operator"
  fi
...
for dir in $(ls -1d "${SCRIPT_DIR}"/[0-9][0-9][0-9]-*/ 2>/dev/null | sort -r); do
  ...
  ns=""
  if [[ "${name}" == "gpu-operator" ]]; then ns="privileged-gpu-operator"; fi
  # Injected mixed-component "-post" folders share their parent's namespace.
  if [[ "${name}" == "gpu-operator-post" ]]; then ns="privileged-gpu-operator"; fi
  ...
  if [[ "${name}" == "gpu-operator-pre" ]]; then ns="privileged-gpu-operator"; fi
  if [[ -z "${ns}" ]]; then
...
kubectl get crd -o json 2>/dev/null \
  | jq -r --arg rel "gpu-operator" --arg ns "privileged-gpu-operator" \
    '.items[] | select(.metadata.annotations["meta.helm.sh/release-name"]==$rel and .metadata.annotations["meta.helm.sh/release-namespace"]==$ns) | .metadata.name' 2>/dev/null \
  | while read -r name; do
      echo "Deleting CRD ${name} (owned by gpu-operator/privileged-gpu-operator)..."
      kubectl delete crd "${name}" --ignore-not-found --wait=false \
        || echo "Warning: failed to delete CRD ${name} (owned by gpu-operator/privileged-gpu-operator); leftovers will surface in post-flight" >&2
    done || echo "Warning: orphan-CRD cleanup for gpu-operator/privileged-gpu-operator failed (kubectl get / jq error); leftovers will surface in post-flight" >&2
...
if [[ "${DELETE_PVCS}" == "true" ]] && ! echo " ${PROTECTED_NS} " | grep -q " privileged-gpu-operator "; then
  echo "Deleting PVCs in privileged-gpu-operator..."
  kubectl delete pvc --all -n privileged-gpu-operator --ignore-not-found || true
fi
delete_orphaned_webhooks_for_ns "privileged-gpu-operator"
delete_namespace "privileged-gpu-operator"
...
if capture_kubectl_json postflight_all_crds_json get crd -o json; then
  ...
  done < <(extra_crds_for_release "gpu-operator")
else
