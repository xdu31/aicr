...
if [[ "${SKIP_PREFLIGHT}" == "true" ]]; then
  ...
  if skip_preflight_for_release "gpu-operator"; then
    echo "  Skipping gpu-operator (gpu-operator): bundle deletes dependent manifests before controller uninstall."
  else
    echo "  Checking gpu-operator (gpu-operator)..."
...
for dir in $(ls -1d "${SCRIPT_DIR}"/[0-9][0-9][0-9]-*/ 2>/dev/null | sort -r); do
  ...
  ns=""
  if [[ "${name}" == "gpu-operator" ]]; then ns="gpu-operator"; fi
  # Injected mixed-component "-post" folders share their parent's namespace.
  if [[ "${name}" == "gpu-operator-post" ]]; then ns="gpu-operator"; fi
  if [[ -z "${ns}" ]]; then
...
kubectl get crd -o json 2>/dev/null \
  | jq -r --arg rel "gpu-operator" --arg ns "gpu-operator" \
    '.items[] | select(.metadata.annotations["meta.helm.sh/release-name"]==$rel and .metadata.annotations["meta.helm.sh/release-namespace"]==$ns) | .metadata.name' 2>/dev/null \
  | while read -r name; do
      echo "Deleting CRD ${name} (owned by gpu-operator/gpu-operator)..."
      kubectl delete crd "${name}" --ignore-not-found --wait=false \
        || echo "Warning: failed to delete CRD ${name} (owned by gpu-operator/gpu-operator); leftovers will surface in post-flight" >&2
    done || echo "Warning: orphan-CRD cleanup for gpu-operator/gpu-operator failed (kubectl get / jq error); leftovers will surface in post-flight" >&2
...
if [[ "${DELETE_PVCS}" == "true" ]] && ! echo " ${PROTECTED_NS} " | grep -q " gpu-operator "; then
  echo "Deleting PVCs in gpu-operator..."
  kubectl delete pvc --all -n gpu-operator --ignore-not-found || true
fi
delete_orphaned_webhooks_for_ns "gpu-operator"
delete_namespace "gpu-operator"
