...
if [[ "${SKIP_PREFLIGHT}" == "true" ]]; then
  ...
  if skip_preflight_for_release "kai-scheduler"; then
    echo "  Skipping kai-scheduler (kai-scheduler): bundle deletes dependent manifests before controller uninstall."
  else
    echo "  Checking kai-scheduler (kai-scheduler)..."
    check_release_for_stuck_crds "kai-scheduler" "kai-scheduler"
  fi
...
kubectl get crd -o json 2>/dev/null \
  | jq -r --arg rel "kai-scheduler" --arg ns "kai-scheduler" \
    '.items[] | select(.metadata.annotations["meta.helm.sh/release-name"]==$rel and .metadata.annotations["meta.helm.sh/release-namespace"]==$ns) | .metadata.name' 2>/dev/null \
  | while read -r name; do
      echo "Deleting CRD ${name} (owned by kai-scheduler/kai-scheduler)..."
      kubectl delete crd "${name}" --ignore-not-found --wait=false \
        || echo "Warning: failed to delete CRD ${name} (owned by kai-scheduler/kai-scheduler); leftovers will surface in post-flight" >&2
    done || echo "Warning: orphan-CRD cleanup for kai-scheduler/kai-scheduler failed (kubectl get / jq error); leftovers will surface in post-flight" >&2
...
if [[ "${DELETE_PVCS}" == "true" ]] && ! echo " ${PROTECTED_NS} " | grep -q " kai-scheduler "; then
  echo "Deleting PVCs in kai-scheduler..."
  kubectl delete pvc --all -n kai-scheduler --ignore-not-found || true
fi
delete_orphaned_webhooks_for_ns "kai-scheduler"
delete_namespace "kai-scheduler"
...
if capture_kubectl_json postflight_all_crds_json get crd -o json; then
  ...
  done < <(extra_crds_for_release "kai-scheduler")
else
