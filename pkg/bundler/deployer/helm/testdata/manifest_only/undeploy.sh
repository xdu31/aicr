...
if [[ "${SKIP_PREFLIGHT}" == "true" ]]; then
  ...
  if skip_preflight_for_release "skyhook-customizations"; then
    echo "  Skipping skyhook-customizations (skyhook): bundle deletes dependent manifests before controller uninstall."
  else
    echo "  Checking skyhook-customizations (skyhook)..."
    check_release_for_stuck_crds "skyhook-customizations" "skyhook"
  fi
...
for dir in $(ls -1d "${SCRIPT_DIR}"/[0-9][0-9][0-9]-*/ 2>/dev/null | sort -r); do
  ...
  ns=""
  if [[ "${name}" == "skyhook-customizations" ]]; then ns="skyhook"; fi
  # Injected mixed-component "-post" folders share their parent's namespace.
  if [[ "${name}" == "skyhook-customizations-post" ]]; then ns="skyhook"; fi
  ...
  if [[ "${name}" == "skyhook-customizations-pre" ]]; then ns="skyhook"; fi
  if [[ -z "${ns}" ]]; then
...
kubectl get crd -o json 2>/dev/null \
  | jq -r --arg rel "skyhook-customizations" --arg ns "skyhook" \
    '.items[] | select(.metadata.annotations["meta.helm.sh/release-name"]==$rel and .metadata.annotations["meta.helm.sh/release-namespace"]==$ns) | .metadata.name' 2>/dev/null \
  | while read -r name; do
      echo "Deleting CRD ${name} (owned by skyhook-customizations/skyhook)..."
      kubectl delete crd "${name}" --ignore-not-found --wait=false \
        || echo "Warning: failed to delete CRD ${name} (owned by skyhook-customizations/skyhook); leftovers will surface in post-flight" >&2
    done || echo "Warning: orphan-CRD cleanup for skyhook-customizations/skyhook failed (kubectl get / jq error); leftovers will surface in post-flight" >&2
...
if capture_kubectl_json postflight_all_crds_json get crd -o json; then
  ...
  done < <(extra_crds_for_release "skyhook-customizations")
else
